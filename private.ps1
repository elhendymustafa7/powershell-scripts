...
param(
    ...
    [string]$ActiveMQinstallationFile = "apache-activemq-5.18.2\bin\win64\InstallService.bat",
...
$Dirs_Paths = @("root","root/backend","root/bot","root/inbox","root/webchat","root/analytics","root/auth","root/integrations")
$App_Details = @(
...
$WebSite_Details = @(
    @{Site = "Private"; Path = "$DefaultAppPath\root" }
    )
$App_Pools = @("backend","inbox","bot","webchatBackend","analytics","auth","integrations")
...
try{  
    ...
    RunBatProgram $ActiveMQinstallationFile
